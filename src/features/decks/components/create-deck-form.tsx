...
import { useCreateDeck } from "../api/use-create-deck";

interface CreateDeckFormProps {
  onCancel: () => void;
}

export const CreateDeckForm = ({ onCancel }: CreateDeckFormProps) => {
  const { mutateAsync, isPending } = useCreateDeck();
      ...
      <CardContent className="w-full space-y-4">
        <CardHeader className="p-0">
          <CardTitle className="flex items-center gap-2">
            ...
            <h1 className="text-2xl">Create Flash Cards Deck</h1>
          </CardTitle>
        </CardHeader>
        <Form {...form}>
